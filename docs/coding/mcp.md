# MCP Servers (Slurm and SU Usage)
...
|---|---|---|---|
| Slurm | `ai-session/mcp/slurm_mcp.py` | `my_jobs`, `job_detail`, `partition_info` | What are my jobs doing? What happened to job N (that I own)? Which partitions are free? |
| SU usage | `ai-session/mcp/su_usage_mcp.py` | `my_usage`, `my_sessions` | How many Service Units have I been billed, by model and GPU tier? What were my individual billed sessions? |
...
Slurm server (`slurm_mcp.py`), all queries scoped to you:
...
`my_usage` returns your total billed SU with a breakdown by model and by GPU tier;
`my_sessions` lists each session's job id, model, tier, reserved wall-hours, SU, and
...
staff-only central ledger at `/project/rcc/mehta5/ai-session-billing`. A normal user
...
- They run only read-only Slurm commands (`squeue`, `sacct`, `sinfo`,
  `scontrol show`). No `scancel`, `sbatch`, `srun`, `scontrol update`, or any other
  mutation is reachable. There is no general "run this command" tool.
...
`qwen3_4b`, not `qwen2.5_coder_32B`: the vLLM `hermes` tool-call parser does not
...
config is correct. Start the session with tool calling enabled and a 72B model, for
example:

```bash
MODEL=qwen2.5_72B TP=4 AGENT_CLIENT=1 \
