...
| Models on disk | Qwen3-4B, Qwen2.5-Coder-32B (added 2026-06-10), Qwen2.5-72B, Llama-3.1-70B, Qwen2.5-0.5B (`models/`) |
...
| Coding model (Coder-32B), 2× A100 | 1.0 × 2 | **2.0** | 4.0 |
...
| You use… | Qwen3-4B (1× A100) | Coder-32B (2× A100) | Qwen2.5-72B (4× A100) | Qwen2.5-72B (4× H100) | Qwen2.5-72B (2× H200) |
|---|---:|---:|---:|---:|---:|
| 1,000 input tokens | 0.000014 | 0.000116 | 0.000383 | 0.000587 | 0.000219 |
| 1,000 output tokens | 0.000067 | 0.000331 | 0.000990 | 0.001227 | 0.000716 |
| A typical chat turn (≈500 in / 300 out) | 0.000027 | 0.00016 | 0.00049 | 0.00066 | 0.00032 |
| A long document (≈8,000 in / 1,000 out) | 0.00018 | 0.00126 | 0.00405 | 0.00592 | 0.00247 |
| One million output tokens | 0.067 | 0.33 | 0.99 | 1.23 | 0.72 |
...
**The coding model (Coder-32B) undercuts the 72B on every tier.** Measured
2026-06-10 (job `50705130`, `midway3-0377`, two 80 GB A100s, TP=2) — and measured
at a **32,768-token** context, the same serve setting coding sessions actually use,
so the price record matches production honestly. It reads ~4,770 and writes ~1,680
tokens/sec, which works out to **0.000331 SU per 1,000 output tokens** — cheaper
than the 72B on *any* GPU (0.000716–0.001227) — while holding at only **2.0
SU/hour** (two A100s instead of the 72B's four). A model built specifically for
code that is also the cheapest way to run code work: that combination is why it
became the coding default (see the 2026-06-10 log entry).
...
- **2026-06-10 — Coding default switched from 72B to a dedicated coding model, and its
  speed test ran GREEN (job 50705130).** A hard look at our coding setup (prompted by
  comparing it against Stanford Sherlock's equivalent service) found it under-built: it
  served the general-purpose 72B chat model with only an 8,192-token context — too small
  to hold much code. Fixes, all now in place: staged **Qwen2.5-Coder-32B** (a model
  trained specifically on code) in `models/`; made it the coding default on **two 80 GB
  A100s** with a **32,768-token context** (the model's full native window — roughly four
  times more room for source files per prompt); and wrote a user guide,
  `ai-session/CODING_AGENTS.md`, covering three ways to connect — **aider** (command
  line, the dependable default), **Continue** (inside VS Code/JetBrains), and
  **opencode** (an experimental autonomous agent) — all through the same stable front
  door. The speed test (job `50705130` on `midway3-0377`, 14 minutes, ~0.46 SU, run at
  the production 32,768-token setting) measured read ~4,770 / write ~1,680 tokens/sec;
  the read/write order and the token double-check both passed, and the price record —
  the fifth in `billing/rate_table.json` — is saved. The economics sealed the decision:
  the Coder-32B is **cheaper per token than the 72B on every GPU tier** (0.000331 vs
  0.000716–0.001227 SU per 1k output) *and* holds at half the 72B's floor (2.0 vs 4.0
  SU/hour), so the better-at-code model is also the cheaper one — a switch with no
  downside for coding work.
- **2026-07-03 —** This file was overwritten by the 2026-06-03 aider test commit and
  restored from git on 2026-07-03.
